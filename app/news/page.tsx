              ...
              <div key={article.id} className="overflow-hidden rounded-2xl bg-card border border-line">
                <div className="relative">
                  {article.imageUrl ? (
                    ...
                    <div className="w-full h-44 flex items-center justify-center text-5xl" style={{ background: CAT_BG[article.category] }}>
                      {article.category === 'local' ? '\uD83C\uDFD8\uFE0F' : article.category === 'state' ? '\uD83C\uDFDB\uFE0F' : '\uD83D\uDDFA\uFE0F'}
                    </div>
                  )}
                  <div className="absolute top-3 left-3">
                    <span className="rounded-full px-2 py-1 text-xs font-bold" style={{ background: CAT_COLORS[article.category].bg, color: CAT_COLORS[article.category].text }}>
                      {CAT_COLORS[article.category].label}
                    </span>
                  </div>
                </div>
                <div className="p-4">
                  ...
                  <Link href={'/news/' + encodeURIComponent(article.id) + '?data=' + encodeURIComponent(JSON.stringify(article))} className="block w-full rounded-xl bg-page border border-terracotta/30 p-3 mb-3 hover:border-terracotta transition-colors">
